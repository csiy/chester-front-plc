    ...
    <v-container fluid>
        <div class="wrapper">
            <div class="container">
                <h1>Welcome</h1>
                <div class="form">
                    <input type="text" placeholder="账号/手机号" v-model.trim="name">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            login() {
                if (this.name == null) {
                    this.$message.error('请输入账号/手机');
                } else if (this.password == null) {
                    this.$message.error('请输入密码');
                } else {
                    AuthApi.login(this.name, this.password).then(v => {
                        this.getUserInfo();
                    }).catch(v => {
            ...
            getUserInfo() {
                UserApi.getUser().then(v => {
                    if(v.data.roles.length===0){
                        this.$message.warning('非管理账号');
                        this.$router.push({path: '/'}).catch(err => err);
                        return;
                    }
                    this.initUser(v.data);
                    this.setLogin(true);
